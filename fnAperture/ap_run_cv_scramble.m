function [ npat, sD ] = ap_run_cv_scramble( incfg, pat )
...
%     };
if 0 == 1
    patCV_file = 'Z:\Jon\proj\cuep\cuep_matlab_data\101\proc_enc_ap_1\ya101enc_ap_bs(none)_c(32)t(45)f(3).mat';
    inData = load(patCV_file); fieldn = fieldnames(inData); pat = inData.(fieldn{1});
    
incfg.rm_ev_field = {...
    'valinv_only',{'~strcmp(e_cond_str, ''neu'') & ~strcmp(e_cond_str, ''req'')'}; %only use valid an invalid trials
    };

incfg.regressor = {'Rscene',{'e_scene_str'}};
incfg.selector = {'e_block_num',{}};
...
if ~isempty(incfg.number_of_scramble) && incfg.number_of_scramble > 0
    ...
    for iRdm = 1:size(random_mat,1)
        ...
        [npat, res] = classify_pat(itpat,SDtmp.proc_label, params);
...
%         sD.dim.freq = pat.dim.freq;
        ...
        for i1 = 1:prod(rS(2:end))
            ...
            if ~isempty(label_str{2})
                ...
                tmpRangeVals = round([npat.dim.time.mat(d2).range],2);
            ...
            if ~isempty(label_str{3})
                ...
                tmpRangeVals = round([npat.dim.freq.mat(d3).range],2);
        ...
        for iP = 1:numel(res.iterations)
            ...
            tmpPerfC{iP,3} = d1;
            tmpPerfC{iP,4} = npat.dim.chan.mat(d2).label;
            tmpPerfC{iP,5} = round(npat.dim.time.mat(d3).range,2);
            tmpPerfC{iP,6} = round(npat.dim.freq.mat(d4).range,2);
        end
