function [freq] = fdtp_ft_freqbaseline(cfg, freq)
% Modified function to include a ztransformation and a true percent change
%  ztransform = (data - meanVals) ./ stdVals;
%  percentchange   = 100*((data - meanVals) ./ meanVals)
% 
...
%   cfg.baselinetype = 'absolute', 'relative', 'relchange', 'normchange',...
%       'db', 'ztransform', or 'percentchange' (default = 'absolute')
...
cfg =               ft_checkopt(cfg, 'baselinetype', 'char', {'absolute', 'relative', 'relchange', 'normchange', 'db', 'vssum','ztransform','percentchange'});
...
if size(baselineTimes,1)==size(data,2)
  ...
  for k = 1:size(baselineTimes,1)
    ...
    stdVals(:,k,:)  = repmat( nanstd(data(:,k,baselineTimes(k,:)),[], 3), [1 1 size(data, 3)]);
  end
else
  meanVals = repmat(nanmean(data(:,:,baselineTimes), 3)   , [1 1 size(data, 3)]);
  stdVals  = repmat( nanstd(data(:,:,baselineTimes),[], 3), [1 1 size(data, 3)]);
end
...
elseif (strcmp(baselinetype, 'ztransform'))
  data = (data - meanVals) ./ stdVals;
elseif (strcmp(baselinetype, 'percentchange'))
  data = 100*((data - meanVals) ./ meanVals);
else
