function [ fdtp_stat ] = fdtp_stats_mean_level( incfg, conCell )
% Within subject helper function
% conCell = 1 x N cell array with each cell containing a data structure
% representing the data for that condition (could be between subjects /
% trials, or within subjects).
...
% For within subject analysis condition 1 vs condition 2, the index numbers
% for conCell{1} and conCell{2} should corrospond to the same participant
% conCell{1}(participant1) conCell{2}(participant1)
...
if ~isfield(incfg,'minNb');incfg.minNb= 2; end
...
if (use_reduced_freq || use_reduced_time)  && (strcmpi(freq_avg,'yes') || strcmpi(time_avg,'yes'))
    ...
    if isstruct(rcfg)
        rCell = cell(1,length(conCell));
        for ii = 1:length(conCell)
            for iii = 1:length(conCell{ii})
                rCell{ii}(iii) = conCell{ii}(iii);
                [ rCell{ii}(iii).data ] = fdtp_reducedata( rcfg, conCell{ii}(iii).data );
            end
        end
    end
    conCell = rCell;
...
for iCon = 1:length(conCell)
    chkCon = conCell{iCon}; %disp(chkCon(1).data)
...
if ~isempty(output_folder_base)
    ...
    if iscell(fdtp_stat.contrast.label) && length(fdtp_stat.contrast.label) > 1
        contrast_label = ['[',strjoin(fdtp_stat.contrast.label,']vs['),']'];
    else
        contrast_label = ['[',fdtp_stat.contrast.label,']'];
    end
    
    file_out_base = [str_tmp,contrast_label];
    
    saveData  = fullfile(out_base,[file_out_base,'.mat']);
    disp(['HP: Saving: ',saveData]); 
    disp(out_base); disp(file_out_base)
    ...
    if ischar(freq_array) && strcmpi(freq_array,'all')
        freq_array = [contrast_data.freq' contrast_data.freq'];
    end
